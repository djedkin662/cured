local Script = "Cured owns you https://discord.gg/JtZF9nGtV"

local Owner = "sigma"
local BlackScreen = true
local DisableRendering = true
local FPSCap = 60
local Guns = {"rifle", "aug", "flintlock", "flamethrower"}
...
if Script ~= "Cured owns you https://discord.gg/JtZF9nGtV" then
    game.Players.LocalPlayer:Kick("Get the new version of cured https://discord.gg/JtZF9nGtV")
    return
end
...
-- Play when character spawns
player.CharacterAdded:Connect(playAnimation)

-- Play immediately if character already exists
if player.Character then
	playAnimation(player.Character)
end

function handleFlameCommand(targetName, specificBot)
    targetName = targetName:lower()
    if specificBot then
        specificBot = specificBot:lower()
    end
...
function setupChatListener(player)
    ...
    activeListeners[player.UserId] = function(msg)
        ...
        if (isAuthorized(player)) then
            ...
            elseif msgLower:match("^%.flame%s+([^%s]+)$") then
                local inputName = msgLower:match("^%.flame%s+([^%s]+)$")
                for _, target in pairs(Players:GetPlayers()) do
                    local name, display = target.Name:lower(), target.DisplayName:lower()
                    if name:find(inputName, 1, true) or display:find(inputName, 1, true) then
                        if isProtected(target) then
            ...
            if targetCharacter then
                ...
                if isKO and trashtalkactive then
                    if not hasSentKOMessage then
                        sendMessage("Cured runs you G G / JtZF9nGtV")
                        sendMessage("Good lord I had no idea there was still people this broke")
                        sendMessage("sweet mother of Bunz, tap into cured JtZF9nGtV")
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if buyingInProgress or buyingGunInProgress or buyingMaskInProgress then return end
    ...
    if not (targetCharacter and playerChar) then return end
    ...
    if flameonly then
        ...
        root.CFrame = CFrame.lookAt(orbitPosition, lookPos)
    else
        local offset = Vector3.new(
            math.random(-20, 20),
            math.random(-20, 20),
...
task.spawn(function()
    while true do
        if flameonly and lockedTarget and not voiding and not (buyingInProgress or buyingGunInProgress or buyingMaskInProgress) then
...
RunService.Heartbeat:Connect(function()
    ...
    for _, tool in ipairs(playerChar:GetChildren()) do
        if tool:IsA("Tool") and tool:FindFirstChild("Handle") and tool.Name ~= "[Flamethrower]" then
...
function getNextItemToBuy()
    ...
    for i = 1, #Guns do
        ...
        if gunInfo then
            if gunKey == "flamethrower" then
                if flameonly and not hasGun(gunInfo.toolName) then
                    return "gun"
                end
            else
                if not hasGun(gunInfo.toolName) then
                    return "gun"
                end
            end
        end
    ...
    while true do
        ...
        if flameonly then
            if getNextItemToBuy() == "gun" then
                local shopName = "[Flamethrower] - $9835"
                local toolName = "[Flamethrower]"
            ...
            if gunInfo and gunKey ~= "flamethrower" and getNextItemToBuy() == "gun" then
                ...
                if ammoCount and ammoCount <= 0 then
                    ...
                    if ammoItem and Character and root and humanoid then
                        ...
                        while purchaseCount < 6 do
                            ...
                            if root then
                                root.CFrame = CFrame.new(ammoItem.Head.CFrame.Position)
                            end
                            fireclickdetector(clickDetector)
...
task.spawn(function()
    while true do
        ...
        if not (buyingInProgress or buyingGunInProgress or buyingMaskInProgress) then
            ...
            if Backpack and humanoid and humanoid.Health > 0 then
                if flameonly then
                    ...
                    for _, gunKey in ipairs(Guns) do
                        if gunKey ~= "flamethrower" then
                            ...
                            if gun and not Character:FindFirstChild(gunName) then
...
task.spawn(function()
    while Heartbeat:Wait() do
        local character = LocalPlayer.Character
        if flameonly and lockedTarget and teleporting and not voiding and character then
            local humanoid = character:FindFirstChildOfClass("Humanoid")
            local root = humanoid and humanoid.RootPart

            if root then
                local isPurchasing = buyingInProgress or buyingGunInProgress or buyingMaskInProgress
